...
void lora_load_keyboard(u16 x,u16 y,u8 **kbtbl)
{
	...
	for(i=0;i<13;i++)
	{
		...
		if(i<11)LCD_ShowString(x+(i%3)*80,y+6+28*(i/3),200,16,16,(u8*)kbd_tbl[i]);
		//Show_Str_Mid(x+(i%3)*80,y+6+28*(i/3),(u8*)kbd_tbl[i],16,80);
		else if(i==11)
			LCD_ShowString(x+(i%3)*80,y+6+28*(i/3),200,16,16,kbd_fn_tbl[i-11]);
...
{		  
    ...
    if((i==3)&&(j==2))
		LCD_ShowString(x+j*80,y+6+28*i,200,16,16,(u8*)kbd_fn_tbl[0]);
		//Show_Str_Mid(x+j*80,y+6+28*i,(u8*)kbd_fn_tbl[0],16,80);
	else if((i==4)&&(j<3)) 
			LCD_ShowString(80,180+6+28*i,200,16,16,kbd_fn_tbl[1]);
	    //Show_Str_Mid(80,180+6+28*i,kbd_fn_tbl[1],16,80); 
    else 
				LCD_ShowString(x+j*80,y+6+28*i,200,16,16,(u8*)kbd_tbl[keyx]);
...
u8* cnttbl_mode[3]={"General", "Wake up", "Low"};
//“透明传输”，“定向传输”
u8* cnttbl_sta[2]={"Trans tr", "Direc tr"};
...
void Menu_ui(void)
{
	...
	LCD_Clear(WHITE);
	POINT_COLOR=RED;
	LCD_ShowString(50,10,200,16,16,"Lora remote control");
	LCD_ShowString(50,30,200,16,16,"Configuration page");
//	Show_Str_Mid(0,30,"ATK-LORA-02 测试程序",16,240); 
	POINT_COLOR=BLUE;
	LCD_ShowString(40,50+20,200,16,16,"RED:next");
	LCD_ShowString(40,70+20,200,16,16,"YELLOW:pre");
	LCD_ShowString(40,90+20,200,16,16,"BLUE:OK or cancel");
...
//	Show_Str(40+30,90+20,200,16,"WK_UP:确定/取消",16,0); 
	...
	LCD_ShowString(45,45+95,200,16,16,"Come comm");
	LCD_ShowString(45,70+95,200,16,16,"Module addr");
	LCD_ShowString(45,95+95,200,16,16,"Comm chan");
	LCD_ShowString(45,120+95,200,16,16,"Air speed:");
	LCD_ShowString(45,145+95,200,16,16,"Trans power:");
	LCD_ShowString(45,170+95,200,16,16,"Work mode:");
	LCD_ShowString(45,195+95,200,16,16,"Send status:");
...
//	Show_Str(40+30,195+95,200,16,"发送状态：",16,0);
	POINT_COLOR=BLUE;
	LCD_ShowString(5,45+95,200,16,16,"TO");
	...
	LCD_ShowString(40+110+10,120+95,200,16,16,(u8*)cnttbl_rate[LoRa_CFG.wlrate]);
	LCD_ShowString(40+110+10,145+95,200,16,16,(u8*)cnttbl_power[LoRa_CFG.power]);
	LCD_ShowString(110+10+20,170+95,200,16,16,(u8*)cnttbl_mode[LoRa_CFG.mode]);
	LCD_ShowString(110+10+20,195+95,200,16,16,(u8*)cnttbl_sta[LoRa_CFG.mode_sta]);
	  ...
	  if(key==KEY1_PRES||key==KEY0_PRES)
	  {
		  switch(num)
		  {
			  case 1: 
					...
					LCD_ShowString(40+110+10,70+95,200,16,16,"                ");
					//Show_Str(40+110+10,70+95,200,16,"                ",16,0);
					LCD_ShowNum(40+110+10,70+95,LoRa_CFG.addr,count,16);
					break;
			 case 2: 
					count = num_count(LoRa_CFG.chn);
							LCD_ShowString(40+110+10,95+95,200,16,16,"                ");
			        //Show_Str(40+110+10,95+95,200,16,"                ",16,0);
			        LCD_ShowxNum(40+110+10,95+95,LoRa_CFG.chn,count,16,0);
					break;
			 case 3:
						LCD_ShowString(40+110+10,120+95,200,16,16,"             ");
						LCD_ShowString(40+110+10,120+95,200,16,16,(u8*)cnttbl_rate[LoRa_CFG.wlrate]);
//				    Show_Str(40+110+10,120+95,200,16,"             ",16,0);
//				    Show_Str(40+110+10,120+95,200,16,(u8*)cnttbl_rate[LoRa_CFG.wlrate],16,0);
					break;
			 case 4:
						LCD_ShowString(40+110+10,145+95,200,16,16,"             ");
						LCD_ShowString(40+110+10,145+95,200,16,16,(u8*)cnttbl_power[LoRa_CFG.power]);
				    //Show_Str(40+110+10,145+95,200,16,(u8*)cnttbl_power[LoRa_CFG.power],16,0);
					break;
			 case 5: 
						LCD_ShowString(20+110+10,170+95,200,16,16,"             ");
						LCD_ShowString(20+110+10,170+95,200,16,16,(u8*)cnttbl_mode[LoRa_CFG.mode]);
				    //Show_Str(40+110+10,170+95,200,16,(u8*)cnttbl_mode[LoRa_CFG.mode],16,0);
				    break;
			 case 6:
						LCD_ShowString(20+110+10,195+95,200,16,16,"             ");
						LCD_ShowString(20+110+10,195+95,200,16,16,(u8*)cnttbl_sta[LoRa_CFG.mode_sta]);
				    //Show_Str(40+110+10,195+95,200,16,(u8*)cnttbl_sta[LoRa_CFG.mode_sta],16,0);
				    break;       
		  }
	  }
...
u8 obj_chn=0;//记录目标信道
const u8* set_mode[2]={"Set address:","Set channel"};
...
void Dire_Set(void)
{
 ...
 RESTART:
	...
	chnnumlen=0;
    POINT_COLOR=BLACK;
LCD_ShowString(40,70,200,16,16,(u8*)set_mode[0]);
LCD_ShowString(40+106,70,200,16,16,"          ");	
	...
	while(1)
	{
		...
		if(key)
		{
			...
			else
			{
				...
				else if(key==12)//确定按键 
				{
					if(mode==1)//目标地址设置
					{
						if(addrnumlen)
						{
							...
							{
								POINT_COLOR=RED;
								LCD_ShowString(40,100,200,16,16,"Address out of range, please reset!");
								//Show_Str(40,100,200,16,"地址超出范围,请重新设置！",16,0); 
								delay_ms(500);
								delay_ms(500);
								LCD_ShowString(40,100,200,16,16,"                        ");
							...
							}else //数据正常
							{
								...
								mode=2;//进入目标信道设置界面
								POINT_COLOR=BLACK;
								LCD_ShowString(40,70,200,16,16,(u8*)set_mode[1]);
						...
						{
							    POINT_COLOR=RED;
							LCD_ShowString(40,100,200,16,16,"Please set the address!");
					...
					}else if(mode==2)//目标信道设置
					{
						if(chnnumlen)
						{
							...
							{
								POINT_COLOR=RED;
								LCD_ShowString(40,100,200,16,16,"Channel out of range, please reset!");
						...
						{
							    POINT_COLOR=RED;
							LCD_ShowString(40,100,200,16,16,"Please set channel");
				...
				LCD_Fill(40+106,70,239,70+16,WHITE);
			 LCD_ShowString(40+106,70,200,16,16,callbuf);
...
void Process_ui(void)
{
	...
	LCD_ShowString(10,20,200,12,12,"Modu addr");
	LCD_ShowString(10,40,200,12,12,"Come comm");
	LCD_ShowString(10,60,200,12,12,"Air speed:");
	LCD_ShowString(10,80,200,12,12,"Trans power:");
	LCD_ShowString(10,100,200,12,12,"Work mode:");
	LCD_ShowString(10,120,200,12,12,"Send status:");
...
//	Show_Str(10,120,200,12,"发送状态：",12,0);
	...
	LCD_ShowString(10+60,60,200,12,12,(u8*)cnttbl_rate[LoRa_CFG.wlrate]);
	LCD_ShowString(10+60,80,200,12,12,(u8*)cnttbl_power[LoRa_CFG.power]);
	LCD_ShowString(10+60,100,200,12,12,(u8*)cnttbl_mode[LoRa_CFG.mode]);
	LCD_ShowString(10+60,120,200,12,12,(u8*)cnttbl_sta[LoRa_CFG.mode_sta]);
...
//	Show_Str(10+60,120,200,12,(u8*)cnttbl_sta[LoRa_CFG.mode_sta],12,0);
	POINT_COLOR=BLUE;
	LCD_ShowString(10,150,200,16,16,"data send：");
	LCD_ShowString(10,240,200,16,16,"data rec：");
	LCD_ShowString(110,20,200,12,12,"BLUE: Main menu");
	LCD_ShowString(110,40,200,12,12,"GREEN: One key return");
...
//	Show_Str(130+15,40,200,12,"KEY1: 发送数据",12,0);
	...
	{
		LCD_ShowString(130+15,60,200,12,12,"KEY0: Setting parameters");
//	  Show_Str(130+15,60,200,12,"KEY0: 设置参数",12,0);
	  POINT_COLOR=BLACK;
		LCD_ShowString(130+15,80,200,12,12,"Addr:");
		LCD_ShowString(130+15,100,200,12,12,"Channel:");
//	  Show_Str(130+15,80,200,12,"目标地址:",12,0);
//      Show_Str(130+15,100,200,12,"目标信道:",12,0);
	  POINT_COLOR=RED;
	  num = num_count(obj_addr);
		LCD_ShowString(130+15+55,80,200,12,12,"                ");
