...
u8 LoRa_Init(void)
{
	  ...
	  {
		 LCD_ShowString(40+30,50+20,200,16,16,"Module is busy, please wait");
		 //Show_Str(40+30,50+20,200,16,"模块正忙,请稍等!!",16,0); 	
		 delay_ms(500);
		 //Show_Str(40+30,50+20,200,16,"                    ",16,0);
		 LCD_ShowString(40+30,50+20,200,16,16,"                    ");
         delay_ms(100);		 
	  }
...
{      
	...
	if(LoRa_CFG.mode_sta == LORA_STA_Tran)//透明传输
	{
		...
		X=Get_Adc_Average(ADC_Channel_5,20);
		Y=Get_Adc_Average(ADC_Channel_5,20);
		sprintf((char*)Tran_Data,"X= %d,Y=%d.",X,Y);
		if(key == KEY2)
		{
			sprintf((char*)Tran_Data,"One key return");
			LCD_ShowString(10,220,240,16,16,Tran_Data);
		}
		u3_printf("%s\r\n",Tran_Data);
		LCD_Fill(0,195,240,220,WHITE);//清除显示
		LCD_ShowString(10,195,240,16,16,Tran_Data);
...
//		}
		date[3] =(X>>8)&0xff;//高位地址
		date[4] = X&0xff;//低位地址
		date[5] =(Y>>8)&0xff;//高位地址
		date[6] = Y&0xff;//低位地址
//		date[3] = X;//
//		date[4] = Y;
		//date[3+1] = Dire_Date[i];
		...
		LCD_ShowString(10,200,240,16,16,wlcd_buff);
...
void LoRa_ReceData(void)
{
	...
	if(USART3_RX_STA&0x8000)
	{
		...
		if(LoRa_CFG.mode_sta==LORA_STA_Tran)//透明传输
		{	
			LCD_ShowString(10,270,240,16,16,USART3_RX_BUF);
		...
		}else if(LoRa_CFG.mode_sta==LORA_STA_Dire)//定向传输
		{
			...
			LCD_ShowString(10,270,240,16,16,rlcd_buff);	
...
void Lora_Test(void)
{
	...
	LCD_ShowString(50,30,200,16,16,"Author: JRF");
	//Show_Str_Mid(0,30,"ATK-LORA-02 测试程序",16,240); 
	
	while(LoRa_Init())//初始化ATK-LORA-02模块
	{
		LCD_ShowString(40+30,50+20,200,16,16,"Module not detected");
	...
	LCD_ShowString(40+30,50+20,200,16,16,"Module detected");
    delay_ms(500); 	
	Menu_ui();//菜单
	...
	while(1)
	{
		...
		if(key)
		{
			LCD_ShowString(5,95+45+netpro*25,200,16,16,"  ");
			...
			}else if(key==WKUP_PRES)//KEY_UP按下
			{
				...
				{
					LCD_ShowString(25,95+45+netpro*25,40,16,16,"l");
					LCD_ShowString(5,95+45+netpro*25,60,16,16,"TO");
...
//					Show_Str(30+10,95+45+netpro*25,200,16,"→",16,0);//指向新条目
					...
					LCD_Fill(25,95+45+netpro*25,40,95+45+netpro*25+20,WHITE);//清除下划线显示
				}
			}
			LCD_ShowString(5,95+45+netpro*25,200,16,16,"TO");
			//Show_Str(30+10,95+45+netpro*25,200,16,"→",16,0);//指向新条目
		}
